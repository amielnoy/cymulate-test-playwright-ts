...
export async function downloadReport(
  ...
  await expect(mainPage.reportAddedSuccessMessage).toBeVisible({
    timeout: GlobalConstants.MEDIUM_TIMEOUT,
  })
  const expectedReportTextLine1 =
    'Module - Web Application FirewallAssessment generated at'
  const expectedReportTextLine2 = `${dateTimeUtility.getCurrentDate()} ${dateTimeUtility.getCurrentTime()}}`
  await expect(
    mainPage.downloadsSubPage.downloadedCsvReportDateAndTimeLabel,
  ).toContainText(expectedReportTextLine1)
